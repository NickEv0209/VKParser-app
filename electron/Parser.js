...
const main = async (settings) => {
  ...
  const getResponse = async () => {
    ...
    page.on("request", (request) => {
      if (
        request.url().includes("https://api.vk.com/method/messages.getHistory")
      ) {
  ...
  const pageGoto = async (pageLink, timeout = 0) => {
    ...
    await page
      .goto(settings.pageLink, { waitUntil: "networkidle2", timeout })
      .catch(async () => {
  ...
  const getLinks = async () => {
    ...
    const processFwdMessage = (mess) => {
      ...
      const links = mess.attachments
        .map((attach) =>
          attach.hasOwnProperty("link") ? attach.link.url : null
        )
        .filter(Boolean);
...
async function updateData(settings) {
  cachedUsers = await main(settings).catch(() => {
